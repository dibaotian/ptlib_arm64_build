...
PTHREAD_MUTEX_RECURSIVE_NP
  ...
  void OpenTraceFile(const char * newFilename)
  {
    ...
    if (strcasecmp(filename, "stderr") == 0)
      ...
      {
		  //trace文件名固定，不包含时间戳，回写则覆盖原文件 by zenghui 2019.11.13
          PTime now;
...
ostream & PTrace::Begin(unsigned level, const char * fileName, int lineNum)
{
  ...
  //2020.6.30 chenzhihua 日志回滚的时候，原有日志会被删除，此时重新创建并写入日志
  if (::access(info.filename, 0) != 0)
    info.OpenTraceFile(info.filename);
